...
class DistilBertModelForClassification(Tokenizer):
    def __init__(self,num_labels,label_words,model_path=None,tokenizer_path = None):
        super().__init__(tokenizer_path = tokenizer_path)
        self.device = 'cpu'
        self.index_to_word = dict({id:val for id,val in enumerate(label_words)})
        
        if model_path is None:
            self.model = AutoModelForSequenceClassification.from_pretrained('distilbert-base-uncased',num_labels)
        else:
    ...
    def set_trainer(self,dataset,shuffle:bool=True,batch_size:int=32,epoch:int=10,lr:float=1e-4):
        ...
        num_training_steps=epoch * len(self.train_loader.dataset)
        self.num_epochs = 8
    ...
    def infer(self,text,beam_width:int=1):
        ...
        encod_text = self.tokenizer(text,max_length = 200,truncation=True, padding=True,return_tensors='pt')
